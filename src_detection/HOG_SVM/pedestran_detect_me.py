...
def load_data_set(logger):
    ...
    # 提取正样本
    pos_dir = os.path.join(pwd, 'SingleObject')
    ...
    test_dir = 'D:/vm_disk/ubuntu_16.04/track/data/2DMOT2015/train/TUD-Stadtmitte/img1/'
...
def test_hog_detect(test, svm_detector, logger):
    ...
    for seq_name in brenchmark:
        ...
        for f in test:
            ...
            rects, scores = hog.detectMultiScale(img, winStride=(8,8), padding=(8,8), scale=1.1)
            dets = []
            ...
            cv2.putText(img, '#%03d' % idx, (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 0, 0), 2)

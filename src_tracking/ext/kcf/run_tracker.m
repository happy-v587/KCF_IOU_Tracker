...
function [precision, fps] = run_tracker(video, kernel_type, feature_type, show_visualization, show_plots)
	...
	case 'all',
        ...
        %save fps and precisions
        mat_path = mfilename('fullpath');
        slash_idx = strfind(mat_path, '\');
        mat_path = mat_path(1:slash_idx(end));
        mat_path = strrep(mat_path, '\', '/');        
        save_path = [mat_path '/draw/all-'  kernel_type '-'  feature_type '.mat'];
        save(save_path, 'mean_precision', 'fps');   
	...
	otherwise
        ...
        mat_path = mat_path(1:slash_idx(end));
        mat_path = strrep(mat_path, '\', '/');        
        save_path = [mat_path '/draw/' video '-' kernel_type '-'  feature_type '.mat'];
        save(save_path, 'precisions', 'fps');        		

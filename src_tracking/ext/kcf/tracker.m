...
for frame = 1:numel(img_files),
    ...
    if frame > 1,
        [pos] = predict(im, pos, model_xf, model_alphaf, window_sz, ...
            features, cell_size, cos_window, kernel);
        [model_xf, model_alphaf] = train(im, pos, frame, yf, ...
            model_xf, model_alphaf, window_sz, features, cell_size, ...
            cos_window, kernel, lambda, interp_factor);
    else
        [model_xf, model_alphaf] = train(im, pos, frame, yf, ...
            'null', 'null', window_sz, features, cell_size, ...
            cos_window, kernel, lambda, interp_factor);
    end
...
function [model_xf, model_alphaf] = train(im, pos, frame, yf, ...
        model_xf, model_alphaf, window_sz, features, cell_size, ...
        cos_window, kernel, lambda, interp_factor)
...
function [pos] = predict(im, pos, model_xf, model_alphaf, window_sz, ...
        features, cell_size, cos_window, kernel)
...
%     figure(2); tmp = imresize(response, 16); imshow(tmp);
